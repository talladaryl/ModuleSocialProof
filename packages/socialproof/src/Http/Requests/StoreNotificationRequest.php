...
class StoreNotificationRequest extends FormRequest
{
    ...
    {
        return [
            'widget_id' => 'required|exists:sp_widgets,widget_id',
            'campaign_id' => 'nullable|exists:sp_campaigns,campaign_id',
            'type' => 'required|in:purchase,signup,visit,custom',
    ...
    {
        // Définir un timestamp par défaut si non fourni
        if (!$this->has('timestamp')) {
            $this->merge(['timestamp' => now()]);
        }

        // Définir une durée d'affichage par défaut
        if (!$this->has('display_duration')) {
            $this->merge(['display_duration' => 5]);
        }
